...
class GroqService {
  static const String _apiKey = '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';
  ...
  Future<String?> generateTutorResponse(String userText) async {
    try {
      final response = await http.post(
        ...
        body: jsonEncode({
          ...
          'messages': [
            {
              'role': 'system',
              'content': 'Eres un tutor amigable y educativo. Responde siempre de manera concisa (no más de 2 o 3 oraciones cortas), clara y motivadora.'
            },
            {
              'role': 'user',
              'content': userText
      ...
      if (response.statusCode == 200) {
        ...
        debugPrint('Error from Groq LLM: ${response.statusCode} - ${response.body}');
      }
    } catch (e) {
  ...
  Future<Map<String, dynamic>?> _transcribeAudio(String filePath) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        debugPrint('Error transcribing: ${response.statusCode} - $responseData');
      }
    } catch (e) {
  ...
  Future<void> _saveToProvisionalJson(Map<String, dynamic> newTranscription) async {
    try {
      final directory = await getApplicationDocumentsDirectory();
